...
<body>
	...
	<div class="row" style="margin: 25px auto; padding-left: 100px;">
		<c:forEach var="Expectation" items="${ExpectationList}">
			...
			<ul class="col-xs-2"
				...
				<li style="height: 360px; width: 250px; border: 1px solid gray;"><img
					alt="eggFire" src="${Expectation.movieTitleCut}"
					style="width: 248px; height: 358px;"></li>
				<li
					style="height: 40px; width: 250px; border: 1px solid gray; padding-top: 10px; text-align: center;">평점 ${Expectation.movieRepleScoreAvg}</li>
				<li
					style="height: 30px; width: 250px; border: 1px solid gray;padding-top: 2px; font-size: 17px;">
					<span class="mg${Expectation.movieGrade}" style="padding:3px; border-radius:5px; margin-left: 5px; margin-right: 5px;">${Expectation.movieGrade}</span> 
					<span>${Expectation.movieName}</span></li>
				<li
				...
				<div class="modal-dialog" style="width: 700px;">
					<div class="modal-content">
						<div class="modal-body">
							<img alt="" src="${Expectation.movieTitleCut }" style="height: 250px; width: 200px; float: left; margin-right: 10px; margin-top: 20px;">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button><div style="margin-top: 40px;font-size:25px;">
							<span class="mg${Expectation.movieGrade}" style="padding:3px; border-radius:5px; margin-left: 10px; margin-right: 5px;">${Expectation.movieGrade}</span>
							<span>${Expectation.movieName}</span></div><hr>
							<div class="topInfo">개봉일:${Expectation.releaseDate}</div>
							<div class="topInfo">감독:${Expectation.movieDirector }</div>
							<div class="topInfo">배우:${Expectation.movieActor}</div>
							<div class="topInfo">장르:${Expectation.movieGenre }</div>
							<div class="topInfo">배급사:${Expectation.movieDistributer}</div>
							<br> <br>
							<div>
								<h4 style="margin-top: 20px;">줄거리</h4>
								${Expectation.movieSummary}
								<hr>
							</div>
							<div>
								<h4>스틸컷</h4>
								<div id="carousel-example-generic${count }"
									...
									<div class="carousel-inner" role="listbox">
										<c:forEach var="steelCutImg" items="${steelCutList}">
											<c:if test="${Expectation.movieIdx==steelCutImg.steelCutMovieIdx}">
											...
											<div class="item ${steelCutCount }" id="item${steelCutCount }"style="margin-left: 87px;">
												<img src="${steelCutImg.steelCutFileName }" alt="yelloMovie" style="height: 600px; width: 500px; float: left; padding-right: 10px;">
											</div>
											</c:if>
										</c:forEach>
									</div>
							...
							<div>
								...
								<br>
								<div class="starRev">
									<span class="starR on" onclick="starScore(1,${Expectation.movieIdx})">별1</span> <span class="starR" onclick="starScore(2,${Expectation.movieIdx})">별2</span>
									<span class="starR" onclick="starScore(3,${Expectation.movieIdx})">별3</span> <span class="starR" onclick="starScore(4,${Expectation.movieIdx})">별4</span>
									<span class="starR" onclick="starScore(5,${Expectation.movieIdx})">별5</span>
									&nbsp;&nbsp;&nbsp;
									<input type="text" name="repleContent" style="width: 460px;" id="content${Expectation.movieIdx}">
										<input type="button" value="등록" class="btn" onclick="repAdd(${Expectation.movieIdx})">
										<input type="hidden" value="${sessionScope.smdto.memberIdx}" name="reMemberIdx" id="member${Expectation.movieIdx}">
										<input type="hidden" value="1" name="reScore"  id="score${Expectation.movieIdx}">
										<input type="hidden" value="${Expectation.movieIdx}" name="reMovieIdx" id="movie${Expectation.movieIdx}">
								</div>
								<div style="overflow-y: scroll;width: 660px; height: 450px; margin-top: 10px;">
								<div id="repList${Expectation.movieIdx}">
								<c:forEach var="rep" items="${movieReple}">
									<div>
									<c:if test="${Expectation.movieIdx==rep.repleMovieIdx}">
